...
class Clicker(object):
    ...
    def make_next_click(self, pred_mask, points, new_point, click_indx, max_clicks):
        assert self.gt_mask is not None
        click = self._get_next_click(pred_mask)

        if click.is_positive:
            points[0, max_clicks - click_indx, 0] = float(click.coords[0])
            points[0, max_clicks - click_indx, 1] = float(click.coords[1])
            points[0, max_clicks - click_indx, 2] = float(click_indx)
            new_point[0, 0, 0] = float(click.coords[0])
            new_point[0, 0, 1] = float(click.coords[1])
            new_point[0, 0, 2] = 1 # 1表示正点击
        else:
            points[0, 2 * max_clicks - click_indx, 0] = float(click.coords[0])
            points[0, 2 * max_clicks - click_indx, 1] = float(click.coords[1])
            points[0, 2 * max_clicks - click_indx, 2] = float(click_indx)
            new_point[0, 0, 0] = float(click.coords[0])
            new_point[0, 0, 1] = float(click.coords[1])
            new_point[0, 0, 2] = 0 # 0表示负点击
