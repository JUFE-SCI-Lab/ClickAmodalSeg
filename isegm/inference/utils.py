...
from isegm.data.datasets.kins import KINSDataset
from isegm.data.datasets.cocoa import COCOADataset
from isegm.data.datasets.d2sa import D2SADataset
...
def get_dataset(dataset_name, cfg):
    if dataset_name == 'GrabCut':
        ...
        dataset = KINSDataset(cfg, mode='test')
    elif dataset_name == 'COCOA':
        dataset = COCOADataset(cfg, mode='test')
    elif dataset_name == 'D2SA':
        dataset = D2SADataset(cfg, mode='test')
    else:
        dataset = None
...
def compute_noc_metric(all_ious, all_ious_occ, counts_occ, iou_thrs, max_clicks=20):
    def _get_noc(iou_arr, iou_thr):
        vals = iou_arr >= iou_thr
        return np.argmax(vals) + 1 if np.any(vals) else max_clicks
    
    def _get_noc_occ(iou_arr_occ, iou_thr):
        if np.all(iou_arr_occ < 0.1):
            scores_arr_occ = 0
        else:
            vals_occ = iou_arr_occ >= iou_thr
            scores_arr_occ = np.argmax(vals_occ) + 1 if np.any(vals_occ) else max_clicks
        
        return scores_arr_occ
    ...
    for iou_thr in iou_thrs:
        ...
        scores_arr_occ = np.array([_get_noc_occ(iou_arr_occ, iou_thr)
                               for iou_arr_occ in all_ious_occ], dtype=int)
        ...
        score_occ = scores_arr_occ.sum() / counts_occ

        noc_list.append(score)
        noc_list_std.append(score_std)
        over_max_list.append(over_max)

        noc_occ_list.append(score_occ)

    return noc_list, noc_occ_list, noc_list_std, over_max_list
...
def get_results_table(noc_list, noc_occ_list, over_max_list, brs_type, dataset_name, mean_spc, elapsed_time,
                      n_clicks=20, model_name=None):
    table_header = (f'|{"BRS Type":^13}|{"Dataset":^11}|'
                # f'{"NoC@80%":^9}|{"NoC@85%":^9}|{"NoC@90%":^9}|{"NoC@95%":^9}|'
                f'{"NoC_occ@80%":^13}|{"NoC_occ@85%":^13}|{"NoC_occ@90%":^13}|'
    ...
    table_row += f'{noc_occ_list[0]:^13.2f}|' 
    table_row += f'{noc_occ_list[1]:^13.2f}|' if len(noc_list) > 1 else f'{"?":^9}|'
    table_row += f'{noc_occ_list[2]:^13.2f}|' if len(noc_list) > 2 else f'{"?":^9}|'
    # table_row += f'{noc_occ_list[3]:^13.2f}|' if len(noc_list) > 3 else f'{"?":^9}|'
