...
class ISModel_prevMod(nn.Module):
    ...
    def forward(self, image, points):
        image, prev_mask, prior_mask = self.prepare_input(image)
        ...
        if torch.all(prev_mask == torch.zeros_like(prev_mask)):
            ...
            coord_features = torch.cat((coord_features, prior_mask), dim=1) # add shape prior
            coord_features = coord_features.float()

            coord_features = self.maps_transform(coord_features)
            # outputs = self.backbone_forward(image, coord_features, prev_mask, prev_mask) # base
            outputs = self.backbone_forward(image, coord_features, prev_mask, prev_mask, prior_mask) # add shape prior
        ...
        coord_features = self.maps_transform(coord_features)
        # outputs = self.backbone_forward(image, coord_features, prev_mask, prev_mask_modulated)  # base
        outputs = self.backbone_forward(image, coord_features, prev_mask, prev_mask_modulated, prior_mask)
    ...
    def prepare_input(self, image):
        ...
        return image, prev_mask, prior_mask
