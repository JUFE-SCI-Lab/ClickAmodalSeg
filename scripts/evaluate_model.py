...
sys.path.insert(0, '.')
sys.path.append('/home/univ/hongren/Projects/Amodal_Interactive_Segmentation')
...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='GrabCut,Berkeley,DAVIS,SBD',
    ...
    group_iou_thresh.add_argument('--target-iou', type=float, default=0.99,
    ...
    parser.add_argument('--log_path', type=str,
                        default="/home/univ/hongren/Projects/Amodal_Interactive_Segmentation/logs", )
...
def main():
    ...
    log_path = os.path.join(args.log_path, '{}'.format(args.dataset))
    logger = setup_logger(os.path.join(log_path, 'logs'), logfile_name=log_file)
    ...
    for dataset_name in args.dataset.split(','):
        ...
        for checkpoint_path in checkpoints_list:
            ...
            model.load_NewAE(cfg, model_path=cfg.autoencoder_path, logger=logger)
            # model.load_QKHead(cfg, model_path=cfg.QKHead_path, logger=logger)
            # model.QKHead.encoder.load_state_dict(model.AE_net.AM_AE_Net.AE.encoder.state_dict())

            # """打印模型各模块的参数量，单位为M"""
            # print(f"{'模块名称':<15} {'参数量 (M)':>15}")
            # print("-" * 30)
            # for name, module in model.named_children():
            #     params = sum(p.numel() for p in module.parameters()) / 1e6
            #     print(f"{name:<15} {params:>15.4f}")
            # print("-" * 30)
            # total = sum(p.numel() for p in model.parameters()) / 1e6
            # print(f"{'总计':<15} {total:>15.4f}")
...
def save_results(args, row_name, dataset_name, logs_path, logs_prefix, dataset_results,
    ...
    all_ious, all_ious_occ, all_ious_occ_c2f, counts_occ, elapsed_time = dataset_results
    mean_spc, mean_spi = utils.get_time_metrics(all_ious, elapsed_time)

    # iou_thrs = np.array([0.80, 0.85, 0.90, 0.95]).tolist()
    iou_thrs = np.array([0.80, 0.85, 0.90]).tolist()

    noc_list, noc_occ_list, noc_list_std, over_max_list = utils.compute_noc_metric(all_ious, all_ious_occ, counts_occ,
                                                                                   iou_thrs=iou_thrs,
    ...
    header, table_row = utils.get_results_table(noc_list, noc_occ_list, over_max_list, row_name, dataset_name,
    ...
    if args.print_ious:
        ...
        miou_str_full = ' '.join([f'mIoU@{click_id}={mean_ious[click_id - 1]:.2%};'
                                  for click_id in range(1, 21) if click_id <= min_num_clicks])

        # mIoU_occ
        min_num_clicks_occ = min(len(x) for x in all_ious_occ_c2f)
        ious_matrix = np.array([x[:min_num_clicks_occ] for x in all_ious_occ_c2f])
        sum_ious = ious_matrix.sum(axis=0)
        epsilon = 1e-8
        mean_ious_occ = sum_ious / (counts_occ + epsilon)

        miou_str_occ = ' '.join([f'mIoUocc@{click_id}={mean_ious_occ[click_id - 1]:.2%};'
                                 for click_id in range(1, 21) if click_id <= min_num_clicks_occ])

        table_row += '; mIoU_full: ' + miou_str_full + ' mIoU_occ: ' + miou_str_occ
    else:
        target_iou_int = int(args.target_iou * 100)
        if target_iou_int not in [80, 85, 90, 95, 100]:
            noc_list, noc_occ_list, noc_list_std, over_max_list = utils.compute_noc_metric(all_ious, all_ious_occ,
                                                                                           counts_occ,
                                                                                           iou_thrs=[args.target_iou],
                                                                                           max_clicks=args.n_clicks)
